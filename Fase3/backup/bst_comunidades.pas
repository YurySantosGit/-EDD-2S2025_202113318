...
procedure BSTC_ListCommunities(T: PBSTC; items: TStrings);
  ...
  begin
    if N = nil then Exit;
    InOrder(N^.L);
    items.Add(N^.nombre);  // <-- nombre del grupo
    InOrder(N^.R);
  end;
begin
...
begin
  ...
  // fecha de creación almacenada en el nodo
  fechaCreacion := N^.fechaCreacion;

  // contamos mensajes recorriendo la lista simple
  M := N^.mensajes.cabeza;   // <-- cabeza de la lista
  while M <> nil do
  begin
    Inc(count);
    M := M^.sig;             // <-- siguiente
...
begin
  ...
  M := N^.msgsHead;   // <-- cabeza de la lista
  while M <> nil do
  begin
