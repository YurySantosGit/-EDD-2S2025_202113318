...
const
  // Árbol B de orden 5
  MAX_KEYS = 4;  // máximo de claves por nodo
  MIN_KEYS = 2;  // mínimo de claves por nodo (excepto raíz)
...
type
  ...
  PBNode = ^TBNode;
  TBNode = record
    n: Integer;                           // # de claves usadas [0..MAX_KEYS]
    key: array[1..MAX_KEYS] of Integer;   // claves (IDs)
    data: array[1..MAX_KEYS] of TFavorito;// datos asociados a cada clave
    child: array[0..MAX_KEYS] of PBNode;  // hasta 5 hijos (orden-1)
    leaf: Boolean;
  end;

  // Procedimiento callback "normal" (no anidado)
...
begin
  ...
  // Para orden 5: z recibe 1 clave (4 - (2+1) = 1)
  z^.n := MAX_KEYS - (MIN_KEYS + 1); // =1

  // Copiar la mitad derecha de y hacia z
  for j := 1 to z^.n do
  begin
  ...
  // y queda con MIN_KEYS claves
  y^.n := MIN_KEYS;

  // Reacomodar hijos del padre y colocar z
...
var
  ...
  i: Integer;
begin
  // Si la clave ya existe, actualizamos el dato y salimos
  ...
  // Raíz llena: partir antes de insertar
...
var
  ...
  i: Integer;
begin
  // Fusiona child[idx-1] (c) + clave idx del padre + child[idx] (s)
  c := n^.child[idx - 1];
  s := n^.child[idx];
...
begin
  ...
  begin
    Merge(n, idx);
    Exit(RemoveInternal(n^.child[idx - 1], id)); // id viene por parámetro
  end;
end;
...
begin
  ...
  // Si la raíz quedó vacía y tiene hijo, bajar un nivel
