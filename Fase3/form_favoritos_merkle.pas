...
{================== Recolectores globales para callback ==================}

var
  gHashes   : TStringList = nil;
  gDetalles : TStringList = nil;
  gCountFav : Integer     = 0;
...
procedure AddDetailGlobal(const F: TFavorito);
var
  h: string;
begin
  h := HashFavoritoG(F);
  if Assigned(gHashes)   then gHashes.Add(h);
  if Assigned(gDetalles) then
    gDetalles.Add(Format('ID:%d | De:%s | Asunto:%s | Fecha:%s | Hash:%s',
...
begin
  ...
  gCountFav := 0;
  try
    BFav_InOrder(FavoritosBTree, @AddDetailGlobal);
    countFav := gCountFav;
  finally
    gHashes := nil;
    gDetalles := nil;
